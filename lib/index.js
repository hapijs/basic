...
internals.implementation = function (server, options) {
    ...
    const wwwAuth = settings.wwwAuth || 'Basic';
    ...
    const scheme = {
        authenticate: function (request, reply) {
            ...
            if (!authorization) {
                return reply(Boom.unauthorized(null, wwwAuth, settings.unauthorizedAttributes));
            }
            ...
            if (!username && !settings.allowEmptyUsername) {
                return reply(Boom.unauthorized('HTTP authentication header missing username', wwwAuth, settings.unauthorizedAttributes));
            }
            ...
            settings.validateFunc(request, username, password, (err, isValid, credentials) => {
                ...
                if (!isValid) {
                    return reply(Boom.unauthorized('Bad username or password', wwwAuth, settings.unauthorizedAttributes), null, { credentials: credentials });
                }
